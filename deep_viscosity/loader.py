...
def create_reg_datasets(batch_size: int, processed_data_path: str, validation_size: float = 0.2, test_size: float = 0.2) -> Tuple[DataLoader, DataLoader, DataLoader]:
    ...
    # train, test split
    train_list, test_list, train_label, test_label = train_test_split(x_list, y_list, test_size=test_size, random_state=0)
    transform_function = transform()
    train_set = DeepViscosityDataset(processed_data_path, train_list, train_label, transform=transform_function)
    test_set = DeepViscosityDataset(processed_data_path, test_list, test_label, transform=transform_function)
...
train_loader, test_loader, valid_loader = create_reg_datasets(1, "data/processed")